 ...
 * **席 7 状態 × 読み取った人 9 通り（チケットなし ＋ チケットの 8 状態）の
 * 63 通り**あり、**どれかを書き忘れると画面が固まる。** だから分岐を 1 か所に
 * 集め、型の上で総当たりになる形にしてある（63 通りすべてに答えが返ることは
 * `resolve.test.ts` が試している）。
 *
 * ```ts
...
> = {
  ...
  /**
   * 空席。**その席に収まる待ちの人がいれば、その人が先である**（7.8 の 7 行目）。
   * 誰も収まらないなら、その場で座れる（7.12 の飛び込み着席）。
   *
   * **実際には、いつも飛び込み着席のほうになる。** 割当がコマンドのたびに必ず
   * 走るので、「収まる人が待っている空席」が残らないからである（不変条件
   * `no_starvation`）。判定を残してあるのは、割当の走らせ方を変えたときに
   * 効かせるためで、いま到達しないことは `resolve.property.test.ts` が見張る。
   */
  FREE: (state, table) =>
