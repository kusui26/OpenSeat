 ...
 *
 * **出さないコマンドは 6 つ**で、どれも 8.1 に率が無いためである。譲る（`PASS`）、
 * 席の変更（`SWAP_TABLE`）、保留（`PAUSE`）、取り消し（`CANCEL`）、人数の変更
 * （`CHANGE_PARTY_SIZE`）、心拍（`HEARTBEAT`）。**このうち取り消しと心拍を
 * 出さないことは、測るときに効く。** 自発的な取り消しも放置（7.9）も起こらない
 * ので、8.3 の「放置・キャンセルの率」は構造的に 0 になる（PR 14 で扱う）。
 *
