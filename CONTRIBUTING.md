...
[`e2e/`](e2e) にあります（[開発プラン 9.12](docs/260916_plan_OpenSeat.md)）。**本物のブラウザで、本物の組み上がりを、本物のサーバに当てます。**

```
pnpm build:apps                            # 先に組み立てる（配る中身が要る）
pnpm --filter @openseat/e2e install:browsers
pnpm --filter @openseat/e2e test
pnpm --filter @openseat/e2e test:ui        # 目で追う
```

**単体でも結合でも捕まえられないのは「配線」です。** CSP、画面の中の道、`fetch`、配信、Service Worker —— どれも組み合わせたときにだけ壊れます。

**サーバはテスト専用のもの**（[`e2e/harness/main.ts`](e2e/harness/main.ts)）を立てます。本番の入口（`apps/server/src/main.ts`）とは別物で、違いは 3 つだけです。

| | 本番 | ハーネス |
|---|---|---|
| 時計 | `Date.now()` | 外から進められる（期限切れの再現。9.12） |
| 刻み | 10 秒ごと | 200 ミリ秒ごと |
| 追加の口 | 無い | `/__test/*`（時計と、卓上の QR） |

**時計を動かせる口を、本番に開けません。** 時刻を動かせるということは、順番を飛ばす・席を取り上げるといったことを外から起こせるということです（[CLAUDE.md 7 章](.claude/CLAUDE.md)）。組み立てているのは PR 6 で切り出した本物の `buildApp` なので、口を足さずに済んでいます。

**筋書きを足すときの決まり。**

- **1 つずつ走らせます**（`workers: 1`）。世界が 1 つしかなく、とくに**時計は全体のもの**なので、並走すると期限を進める筋書きがほかの人の順番を奪います
- **使った席は返します。** `world.join()` が覚えていて、筋書きが終わると取り消します。返さないと、筋書きが増えたあるとき突然、席が足りなくなって落ちます
- **配信で届いたことを確かめるときは `BY_STREAM` と `withoutAsking` を組で使います。** 待ち時間だけでは、退避のポーリングがたまたま間に合って素通りします（実際に素通りしました）
- **文言は鍵で引きます。** 日本語を直書きすると、文言を直したときに「壊れた」のか「変えた」のか分からなくなります
- **CI の実行時間は 5 分以内に保ちます**（Phase 2 プラン PR 8）。超えるなら筋書きを減らすこと
