...
**SSE 1 本で配り、つながらないあいだは 5 秒ポーリングに退避します**（[ADR-0018](adr/0018-server-sent-events.md)。当初は WebSocket を既定としていましたが、**いまの用途がすべて一方向**なので改めました）。

- トピック: `ticket:{id}`（本人の状態）、`venue:{id}:public`（ボード・空き状況）、`venue:{id}:staff`（スタッフ・管理。認証が入る PR 12 から）。
- **流すのはイベントではなく「いまの姿」** —— `GET` で取るのとまったく同じ形を送ります。画面がイベントを状態に畳み込むと、それは状態機械を画面に置くことになるためです。
- **つないだ直後に必ず現在の姿を 1 通送る**ので、切れているあいだの変化は次の接続が吸収します（「どこから追いつくか」を決める必要がありません）。
- 切れたら待ち時間を倍にしながらつなぎ直し、そのあいだは 5 秒ごとに取りに行きます。
- 利用者ページの接続をハートビートとして扱い、7.9 の放置判定に使います。
- 配信量は小さい（施設あたり数百接続、数十イベント/分）。**前と同じ姿は送りません。**
...
- **どちらも画面が作り、サーバはハッシュだけを保存する。** 秘密パラメータをサーバが作ると、**送り直しに同じ値を返せない**（冪等キーの控えは結末だけで、応答を持たないため。[ADR-0015](adr/0015-idempotency-key.md)）。画面が作れば、1 回目の応答が失われても手元に残る。
- **Cookie だけで状態を変える入口は作らない。** Cookie は控えで、`localStorage` が消えた端末が自分のチケットを見つけ直すために置く。書き込みは `Origin` を照合して、よそのサイトからのものを断る（CSRF 対策）。
